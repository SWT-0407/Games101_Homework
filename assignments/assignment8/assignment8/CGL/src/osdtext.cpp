...
int OSDText::init(bool use_hdpi) {
  ...
  // MSVC 2019 cannot compile the generated, very long Base64 string reliably.
#ifdef _WIN32
  const char* windows_dir = std::getenv("WINDIR");
  string font_path = string(windows_dir ? windows_dir : "C:\\Windows") + "\\Fonts\\arial.ttf";
  if(FT_New_Face(*ft, font_path.c_str(), 0, face)) {
    out_err("Cannot open Windows system font");
    return -1;
  }
#else
